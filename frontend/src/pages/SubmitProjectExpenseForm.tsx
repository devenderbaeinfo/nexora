...
const CATEGORIES = ["Travel", "Material", "Vendor", "Software", "Other"];

interface ProjectOption { id: string; name: string; }

export default function SubmitProjectExpenseForm({ onDone }: { onDone: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api.post("/project-expenses", {
      projectId, amount: Number(amount), category, description, incurredOn, isBillable,
    }),
    onSuccess: () => {
  ...
  return (
    <form onSubmit={onSubmit}>
      <label style={s.label} htmlFor="project">Project</label>
      <select id="project" style={s.field} value={projectId} onChange={(e) => setProjectId(e.target.value)} required>
      ...
      <label style={{ ...s.label, display: "flex", alignItems: "center", gap: 8, textTransform: "none", letterSpacing: 0, fontSize: 13 }}>
